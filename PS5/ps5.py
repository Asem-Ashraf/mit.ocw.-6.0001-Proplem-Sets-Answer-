...
class TitleTrigger(phrasetrigger):
    def __init__(self, triggertitle):
        phrasetrigger.__init__(self, triggertitle)
...
class DescriptionTrigger(phrasetrigger):
    def __init__(self, triggerdescription):
        phrasetrigger.__init__(self, triggerdescription)
...
class TimeTrigger(Trigger):
    def __init__(self, triggertime):
        self.date_string = (datetime.strptime(triggertime, "%d %b %Y %H:%M:%S")).replace(tzinfo=pytz.timezone("EST"))
...
class BeforeTrigger(TimeTrigger):
    def __init__(self,triggertime):
        TimeTrigger.__init__(self,triggertime)
    def evaluate(self, story):
        return self.date_string>(story.pubdate).replace(tzinfo=pytz.timezone("EST"))
...
def read_trigger_config(filename):
    ...
    for line in lines:
        data = re.split(",",line)
        if data[0]!= 'ADD':
            if (data[1]=='TITLE')|(data[1]=='DESCRIPTION')|(data[1]=='AFTER')|(data[1]=='BEFORE')|(data[1]=='NOT'):
                trigger_dict[data[0]]=dicty[data[1]](data[2])
            elif (data[1]=='AND')|(data[1]=='OR'):
...
SLEEPTIME = 3  # seconds -- how often we poll
...
def main_thread(master):
    ...
    try:
        # t1 = TitleTrigger("fred")
        # t2 = DescriptionTrigger("airport")
